...
<h1><img src="capLogo.jpg" alt="CAP logo" width="90" height="30"/> 
 Saved</h1>
<p>The draft alert has been saved to the drafts directory as 
<% HashMap<String, String> parms = (HashMap<String, String>)session.getAttribute("parms");
 %>capPathFileName: <%= parms.get("capPathFileName") %>
...
<tbody>
  ...
  <tr><td title="restriction has the rule for limiting distribution of a restricted alert">
    &nbsp;<b>restriction</b>&nbsp;<%= parms.get("capRestriction") %></td></tr> 
  <tr><td title="addresses are e-mail for intended recipients of a private alert">
    &nbsp;<b>addresses</b>&nbsp;<%= parms.get("capAddresses") %></td></tr> 
  <tr><td>
    <table border="1"><tbody><tr>
      <td title="language indicates the natural language used for the alert text" width="33%">
      &nbsp;<b>language</b>&nbsp;<%= parms.get("capLanguage") %></td>
      <td title="category may have values: Geo, Met, Safety, Security, Rescue, Fire, Health, Env, Infra, Other" width="33%">
      &nbsp;<b>category</b>&nbsp;<%= parms.get("capCategory") %></td>
      <td title="responseType may have values: Shelter, Evacuate, Prepare, Execute, Monitor, None" width="33%">
      &nbsp;<b>responseType</b>&nbsp;<%= parms.get("capResponseType") %></td>
    </tr></tbody></table>
  </td></tr>
  ...
  <tr><td>
    <table border="1"><tbody><tr>
      <td title="onset is when the event is expected to begin" width="50%">
      &nbsp;<b>onset</b>&nbsp;<%= parms.get("capOnset") %></td>
      <td title="expires is when the message will be no longer in effect" width="50%">
      &nbsp;<b>expires</b>&nbsp;<%= parms.get("capExpires") %></td>
    </tr></tbody></table>
  </td></tr> 
...
<table><tr>
  ...
  <td><form method="post" action="EditController" id="sendAlertForm">
  <input name="editStep" value="sendAlert" type="hidden" />
  <input value="E-mail this draft alert to me " onclick="sendAlert()" 
         type="button" id="sendAlertButton" />
 </form></td>
 
 <td><form method="post" action="EditController" id="sendAlertForm">
  <input name="editStep" value="publishAlert" type="hidden" />
  <input value="RSS" 
         type="button"  />
 </form></td>
 
 
 <td><form method="post" action="EditController">
  <input name="capPathFileName" value="<%= parms.get("capPathFileName") %>" type="hidden" />
  <input name="editStep" value="draftAlert" type="hidden" />
  <input type="submit" value="Continue editing this draft alert" />
 </form></td>
 <td><form method="post" action="EditController">
  <input name="editStep" value="newAlert" type="hidden" />
  <input type="submit" value="Continue editing, different alert" />
 </form></td>
...
function sendAlert() {
	if (!submitDone) {
		submitDone = true;
		document.getElementById("sendAlertButton").value = 'Sending e-mail, please wait...';
		document.getElementById("sendAlertButton").disabled = true;
	} 
	try { 
		document.getElementById("sendAlertForm").submit();
	}
	catch ( e ) { 
		alert ( e ); 
	} 
